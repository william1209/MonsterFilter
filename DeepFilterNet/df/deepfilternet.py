...
class ModelParams(DfParams):
    ...
    def __init__(self):
        ...
        self.sk_reduction: int = config(
            "SK_REDUCTION", cast=int, default=16, section=self.section
        )
        self.sk_min_width: int = config(
            "SK_MIN_WIDTH", cast=int, default=32, section=self.section
        )
        self.sk_kernels: List[int] = config(
            "SK_KERNELS", 
            cast=Csv(int), 
            default=(3, 5), 
...
class DfOutputReshapeOld(nn.Module):
    ...
    def forward(self, coefs: Tensor) -> Tensor:
        # [B, T, F, O*2] -> [B, T, O, F, 2]
        b, t = coefs.shape[:2]
        coefs = coefs.contiguous().view(b, t, self.df_order, 2, self.df_bins).transpose(3, 4)
        return coefs
